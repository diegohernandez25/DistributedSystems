package Main;

/**
* Parameters for the deployment of Assignment 2 on local or remote machines.
*/
public class Parameters {
  /**
  * General Repository Information host name
  */
  public final static String griHost = "l040101-ws01.ua.pt";

  /**
  * General Repository Information port number
  */
  public final static int griPort = 22469;

  /**
  * Lounge host name
  */
  public final static String loungeHost = "l040101-ws05.ua.pt";

  /**
  * Lounge port number
  */
  public final static int loungePort = 22463;

  /**
  * Park host name
  */
  public final static String parkHost = "l040101-ws03.ua.pt";

  /**
  * Park port number
  */
  public final static int parkPort = 22467;

  /**
  * Outside World host name
  */
  public final static String owHost = "l040101-ws02.ua.pt";

  /**
  * Outside World port number
  */
  public final static int owPort = 22461;

  /**
  * Repair Area host name
  */
  public final static String raHost = "l040101-ws04.ua.pt";

  /**
  * Repair Area port number
  */
  public final static int raPort = 22462;

  /**
  * Supplier Site host name
  */
  public final static String ssHost = "l040101-ws05.ua.pt";

  /**
  * Supplier Site port number
  */
  public final static int ssPort = 22464;

  /**
  * Manager host name
  */
  public final static String managerHost = "l040101-ws06.ua.pt";

  /**
  * Mechanic host name
  */
  public final static String mechanicHost = "l040101-ws09.ua.pt";

  /**
  * Customer host name
  */
  public final static String customerHost = "l040101-ws08.ua.pt";

  /**
  * Total number of Customers
  */
  public final static int numCustomers = 30;

  /**
  * Total number of Mechanics
  */
  public final static int numMechanics = 2;

  /**
  * Total number of Replacement Cars
  */
  public final static int numReplacementCars = 3;

  /**
  * Total number of Car Parts types
  */
  public final static int numCarTypes = 3;

  /**
  * Car parts
  */
  public final static int[] carParts = {0, 0, 0};

  /**
  * Maximum number of storage for each Car Part
  */
  public final static int[] maxCarParts = {1, 1, 1};
}
